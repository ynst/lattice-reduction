...
unsigned M = 10000; // number of locations
unsigned N = 10000; // number of clients
unsigned width = 5000; // width and height of the square
unsigned random_seed = 1; // seed for random number generator
double lower_ratio = 0.01; // lowest facility cost, as a proportion of sqrt{n}.
double upper_ratio = 0.01; // highest facility cost, as a proportion of sqrt{n}.
...
// implementing Lemma 1 in the paper
// edited
						 ...
						 unsigned **sorted_location, unsigned *degree)
{
   // variable declarations
   unsigned i, j; // general-purpose counters
   unsigned k; // for inserting the sorted locations 
   ...
   {
      // find the facility i that minimises f_i + c_{ij}
      // and assing the cost for that facilty to best_cost
      best_cost = BIG_INT;
      for (i = 1; i <= M; i++)
		  if (facility_cost[i] + assignment_cost[i][j] < best_cost)
			  best_cost = facility_cost[i] + assignment_cost[i][j];
...
unsigned * facility_x; // x co-ordinates of locations
unsigned * facility_y; // y co-ordinates of locations
unsigned * client_x; // x co-ordinates of clients
unsigned * client_y; // y co-ordinates of clients
...
try
{
   facility_x = new unsigned[M+1];
   facility_y = new unsigned[M+1];
   client_x = new unsigned[N+1];
   client_y = new unsigned[N+1];
...
for (i=1;i<=M;i++)
{   
	facility_x[i]= (unsigned)((width+1)*rand()/(RAND_MAX+1.0));
	facility_y[i]= (unsigned)((width+1)*rand()/(RAND_MAX+1.0));
}

for (j=1;j<=N;j++)
{   
	client_x[j]= (unsigned)((width+1)*rand()/(RAND_MAX+1.0));
	client_y[j]= (unsigned)((width+1)*rand()/(RAND_MAX+1.0));
}
